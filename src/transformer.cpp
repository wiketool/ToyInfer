...
const float* Transformer::forward(uint32_t token_id, uint32_t pos) {
    ...
    if (state.graph_d == nullptr) {
        ...
        for (int i = 0; i < llmconfig.num_hidden_layers; ++i) {
            ...
            bf16* key_ptr = state.key_cache_d +
                            (i * options.max_seq_len * kv_dim) + (pos * kv_dim);
            bf16* val_ptr = state.val_cache_d +
                            (i * options.max_seq_len * kv_dim) + (pos * kv_dim);
            cudaStreamWaitEvent(state.stream_d[1], state.event_d[0]);
            cudaStreamWaitEvent(state.stream_d[2], state.event_d[0]);
            gemv_proj_bf16<NUM_THREADS>(
                layer_ref.attention.k_proj_d, state.residual_d, key_ptr, kv_dim,
                llmconfig.hidden_size, state.stream_d[1]);
            gemv_proj_bf16<NUM_THREADS>(
                layer_ref.attention.v_proj_d, state.residual_d, val_ptr, kv_dim,
                llmconfig.hidden_size, state.stream_d[2]);
            // qk norm
            ...
            rope_bf16(state.q_d, state.inv_freq_d, pos,
                      ...
                      state.stream_d[0]);
            cudaEventRecord(state.event_d[0], state.stream_d[0]);
            rope_bf16(key_ptr, state.inv_freq_d, pos,
