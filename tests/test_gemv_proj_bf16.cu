...
bool run_case(const gemv_test::Case& test_case) {
    ...
    toyinfer::gemv_proj_bf16<gemv_test::kThreads>(dev_w.get(), dev_x.get(),
                                                  dev_y.get(), test_case.M,
                                                  test_case.N);
...
int main() {
    ...
    const std::vector<gemv_test::Case> cases = {
        gemv_test::make_case("gemv proj bf16 tiny", 1, 2, 4, 0.17f, 0.01f),
        gemv_test::make_case("gemv proj bf16 single-iteration", 9, 256, 12, 0.07f,
                             0.006f),
        gemv_test::make_case("gemv proj bf16 multi-loop", 513, 514, 520, 0.05f,
                             0.004f),
    };
