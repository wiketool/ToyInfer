...
int main() {
    ...
    std::vector<float> host_w(M * N), host_x(N);
    ...
    TCUDA_CHECK(cudaMemcpy(dev_w, w_bf16.data(), sizeof(toyinfer::bf16) * M * N,
                           cudaMemcpyHostToDevice));
    TCUDA_CHECK(cudaMemcpy(dev_x, x_bf16.data(), sizeof(toyinfer::bf16) * N,
                           cudaMemcpyHostToDevice));

    toyinfer::attn_single_proj_bf16<256>(dev_w, dev_x, dev_y, M, N);
    TCUDA_CHECK(cudaDeviceSynchronize());

    std::vector<toyinfer::bf16> out_bf16(M);
    TCUDA_CHECK(cudaMemcpy(out_bf16.data(), dev_y, sizeof(toyinfer::bf16) * M,
    ...
    for (uint32_t i = 0; i < M; ++i) {
        if (!kernel_test::nearly_equal(out[i], expected[i], 8e-2f, 8e-2f)) {
            std::cerr << "[FAIL] attn_single_proj mismatch at row " << i
    ...
    std::cout << "[PASS] attn_single_proj_bf16" << std::endl;
    return 0;
}
