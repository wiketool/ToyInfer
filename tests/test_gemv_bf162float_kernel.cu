...
void launch_kernel(const toyinfer::bf16* W, const toyinfer::bf16* x, float* y,
                   uint32_t M, uint32_t N) {
    dim3 block_dim{NUM_THREADS};
    dim3 grid_dim{M};
    toyinfer::gemv_bf162float_kernel<NUM_THREADS><<<grid_dim, block_dim>>>(
        W, x, y, M, N);
...
int main() {
    ...
    const std::vector<gemv_test::Case> cases = {
        gemv_test::make_case("gemv float kernel tiny", 3, 2, 5, 0.19f, 0.02f),
        gemv_test::make_case("gemv float kernel multi-loop", 17, 514, 21, 0.09f,
                             0.005f),
        gemv_test::make_case("gemv float kernel lmhead-shape-like", 257, 1024, 260,
                             0.04f, 0.003f),
    };
    ...
    std::cout << "[PASS] gemv_bf162float_kernel" << std::endl;
    return 0;
}
