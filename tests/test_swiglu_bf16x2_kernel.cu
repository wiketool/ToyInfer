...
void launch_kernel(const toyinfer::bf16* gate, const toyinfer::bf16* up,
                   toyinfer::bf16* intermedia, uint32_t size) {
    dim3 block_dim{NUM_THREADS};
    dim3 grid_dim{(size + block_dim.x * 2 - 1) / (block_dim.x * 2)};
    toyinfer::swiglu_bf16x2_kernel<<<grid_dim, block_dim>>>(gate, up, intermedia,
                                                            size);
...
bool run_case(const swiglu_test::Case& test_case) {
    ...
    launch_kernel<swiglu_test::kThreads>(dev_gate.get(), dev_up.get(),
...
int main() {
    ...
    tiny.label = "swiglu kernel tiny";
    tiny.size = 2;
    tiny.buffer_size = 6;
    tiny.gate = {-2.0f, 1.5f, 0.25f, -1.25f, 0.5f, -0.75f};
    tiny.up = {0.75f, -0.5f, 1.0f, -1.5f, 0.25f, -0.125f};
    cases.push_back(tiny);
    cases.push_back(swiglu_test::make_case("swiglu kernel partial-block", 514,
                                           520, 0.08f, 0.05f));
    cases.push_back(swiglu_test::make_case("swiglu kernel multi-block", 2048,
                                           2054, 0.021f, 0.017f));
    ...
    std::cout << "[PASS] swiglu_bf16x2_kernel" << std::endl;
    return 0;
}
