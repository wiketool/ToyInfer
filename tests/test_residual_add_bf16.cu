...
bool run_case(const add_test::AddCase& test_case) {
    ...
    toyinfer::residual_add_bf16<add_test::kThreads>(dev_residual.get(),
                                                    dev_hidden.get(),
                                                    test_case.size);
    ...
    return kernel_test::expect_vector_close(out, expected, 2e-2f, 2e-2f,
                                            test_case.label + " wrapper second");
}
...
int main() {
    ...
    cases.push_back(add_test::make_case("residual_add partial-block", 514, 520,
                                        false, 0.09f, 0.07f, 0.6f));
    cases.push_back(add_test::make_case("residual_add multi-block", 1536, 1542,
                                        true, 0.05f, 0.04f, -0.45f));
