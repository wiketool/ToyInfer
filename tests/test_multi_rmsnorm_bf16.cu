...
int main() {
    ...
    cases.push_back(multi_rmsnorm_test::make_case("multi_rmsnorm wrapper single-head",
                                                  1, 64, 68, 1e-5f, false,
                                                  0.13f, 0.09f));
    cases.push_back(multi_rmsnorm_test::make_case(
        "multi_rmsnorm wrapper out-of-place", 4, 128, 4 * 128 + 6, 1e-5f, false,
        0.09f, 0.05f));
    cases.push_back(multi_rmsnorm_test::make_case(
        "multi_rmsnorm wrapper in-place", 5, 64, 5 * 64 + 4, 1e-3f, true, 0.17f,
        0.03f));
    auto zero_case = multi_rmsnorm_test::make_case("multi_rmsnorm wrapper zero-input",
                                                   3, 64, 3 * 64 + 8, 1e-4f,
                                                   false, 0.07f, 0.04f);
